...
async function getMeasurementData(userId: any, startDate: any, endDate: any) {
  ...
  try {
    ...
    const result = await client.query(
      `SELECT TO_CHAR(entry_date, 'YYYY-MM-DD') AS entry_date, weight, ${circumferenceColumns}, steps, height, body_fat_percentage, muscle_mass_kg, bone_mass_kg, body_water_percentage, bmr FROM check_in_measurements WHERE user_id = $1 AND entry_date BETWEEN $2 AND $3 ORDER BY entry_date`,
      [userId, startDate, endDate]
    );
    return result.rows;
  } finally {
